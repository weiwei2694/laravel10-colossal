...
@section('content')
    <section class="border bg-white rounded shadow-sm">
        <div class="p-[20px] border-b">
            <h1 class="font-semibold text-black text-lg">New Testimonial</h1>
        </div>
        <div class="p-[20px]">
            <form action="{{ route('dashboard.testimonials.store') }}" method="POST" class="flex flex-col gap-[20px]"
                ...
                <div class="flex flex-col">
                    ...
                    <textarea name="content" id="content" placeholder="content"
                        class="@error('content') dashboard-input__error @else dashboard-input @enderror" required rows="8">{{ old('content') }}</textarea>
                    @error('content')
                        <span class="text-red-600 text-sm">{{ $message }}</span>
                    @enderror
                </div>
