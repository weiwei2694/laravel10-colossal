...
@section('content')
    <section class="border bg-white rounded shadow-sm">
        <div class="p-[20px] border-b">
            <h1 class="font-semibold text-black text-lg">All Testimonials</h1>
        </div>
        <div class="p-[20px]">
                                    ...
                                    @foreach ($testimonials as $testimonial)
                                        <tr>
                                            ...
                                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-800">
                                                {{ $testimonial->company_name }}</td>
                                            <td class="px-6 py-4 whitespace-nowrap text-start text-sm font-medium">
                                                <div class="flex items-center gap-x-[20px]">
                                                    <div>
                                                        <form id="delete-form"
                                                            action="{{ route('dashboard.testimonials.destroy', '__id__') }}"
                                                            method="POST">
                                                            @csrf
                                                            @method('DELETE')
                                                        </form>

                                                        <button onclick="deletePost({{ $testimonial->id }})" type="button"
                                                    ...
                                                    <x-action-item
                                                        url="{{ route('dashboard.testimonials.show', $testimonial->id) }}">View</x-action-item>
                                                    <x-action-item
                                                        url="{{ route('dashboard.testimonials.edit', $testimonial->id) }}">Edit</x-action-item>
                                                </div>
                                            </td>
                                        </tr>
                                    @endforeach
