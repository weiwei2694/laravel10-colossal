...
@section('content')
    <section class="border bg-white rounded shadow-sm">
        <div class="p-[20px] border-b">
            <h1 class="font-semibold text-black text-lg">New Faq Category</h1>
        </div>
        <div class="p-[20px]">
            <form action="{{ route('dashboard.faq-categories.store') }}" method="POST" class="flex flex-col gap-[20px]">
                @csrf
