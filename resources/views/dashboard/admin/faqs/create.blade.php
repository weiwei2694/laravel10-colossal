...
@section('content')
    <section class="border bg-white rounded shadow-sm">
        ...
        <div class="p-[20px]">
            <form action="{{ route('dashboard.faqs.store') }}" method="POST" class="flex flex-col gap-[20px]">
                @csrf
                ...
                <div class="flex flex-col">
                    ...
                    <textarea name="answer" id="answer" placeholder="Answer"
                        class="@error('answer') dashboard-input__error @else dashboard-input @enderror" required rows="8">{{ old('answer') }}</textarea>
                    @error('answer')
                        <span class="text-red-600 text-sm">{{ $message }}</span>
                    @enderror
                </div>
