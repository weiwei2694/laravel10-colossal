    ...
    <section class="relative max-w-[1025px] mx-auto sm:pt-[150px] pt-[75px] pb-[75px]">
        <div class="flex justify-start lg:justify-between items-start lg:items-center flex-col lg:flex-row gap-y-[75px]">
            ...
            <div
                ...
                <form class="flex flex-col gap-y-[26px]" action="{{ route('quote.store') }}" method="POST">
                    @csrf
                    ...
                    <div class="flex gap-x-[20px] flex-col sm:flex-row gap-y-[26px] sm:gap-y-0">
                        ...
                        <div class="flex flex-col gap-y-[6px] w-full">
                            ...
                            <select name="company_size" id="company_size"
                                class="input-quote @error('company_size') input-error @enderror" required>
                                @foreach ($company_sizes as $company_size)
                                    <option class="text-card-dark" value="{{ $company_size }}"
                    ...
                    <div class="flex flex-col gap-y-[6px] w-full">
                        <label for="problem" class="label-quote">Tell Us Your Problem</label>
                        <textarea name="problem" id="problem" class="input-quote resize-none @error('problem') input-error @enderror"
                            rows="8">{{ old('problem') }}</textarea>
                        @error('problem')
                            <span class="text-red-500 text-xs">{{ $message }}</span>
                        @enderror
                    </div>
